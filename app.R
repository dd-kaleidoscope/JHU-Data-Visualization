    ...
    dashboardBody(
        tabItems(
            ...
            tabItem(tabName = "Esub1",
                    selectInput("select_Esub1", label = h3("Filter by region"), 
                                choices = list("SOUTH ASIA" = "SA", "SUB-SAHARAN" = "SSA", "EUROPE AND CENTRAL ASIA" = "ECA",
                                               "ARAB STATES" = "AS", "LATIN AMERICA AND THE CARIBBEAN" = "LAC", "EAST ASIA AND THE PACIFIC" = "EAP"), 
                                selected = "SA"),
                    checkboxInput("points_Esub1", label = "Show points", value = FALSE),
                    checkboxInput("Esub1legend", label = "Show legends", value = TRUE),
                    plotlyOutput("plot_se_f")
            ),
            tabItem(tabName = "Esub2",
                    selectInput("select_Esub2", label = h3("Filter by region"), 
                                choices = list("SOUTH ASIA" = "SA", "SUB-SAHARAN" = "SSA", "EUROPE AND CENTRAL ASIA" = "ECA",
                                               "ARAB STATES" = "AS", "LATIN AMERICA AND THE CARIBBEAN" = "LAC", "EAST ASIA AND THE PACIFIC" = "EAP"), 
                                selected = "SA"),
                    checkboxInput("points_Esub2", label = "Show points", value = FALSE),
                    checkboxInput("Esub2legend", label = "Show legends", value = TRUE),
                    plotlyOutput("plot_se_m")
            ),
            tabItem(tabName = "Esub3",
                    selectInput("select_Esub3", label = h3("Filter by region"), 
                                choices = list("SOUTH ASIA" = "SA", "SUB-SAHARAN" = "SSA", "EUROPE AND CENTRAL ASIA" = "ECA",
                                               "ARAB STATES" = "AS", "LATIN AMERICA AND THE CARIBBEAN" = "LAC", "EAST ASIA AND THE PACIFIC" = "EAP"), 
                                selected = "SA"),
                    checkboxInput("points_Esub3", label = "Show points", value = FALSE),
                    checkboxInput("Esub3legend", label = "Show legends", value = TRUE),
                    plotlyOutput("plot_pr_f")
            ),
            tabItem(tabName = "Esub4",
                    selectInput("select_Esub4", label = h3("Filter by region"), 
                                choices = list("SOUTH ASIA" = "SA", "SUB-SAHARAN" = "SSA", "EUROPE AND CENTRAL ASIA" = "ECA",
                                               "ARAB STATES" = "AS", "LATIN AMERICA AND THE CARIBBEAN" = "LAC", "EAST ASIA AND THE PACIFIC" = "EAP"), 
                                selected = "SA"),
                    checkboxInput("points_Esub4", label = "Show points", value = FALSE),
                    checkboxInput("Esub4legend", label = "Show legends", value = TRUE),
...
server <- function(input, output, session) {
    ...
    output$plot_se_f = renderPlotly({
        ...
        graph_se_f = ggplot(data = data_se_f %>% 
                                filter(data_se_f$region==input$select_Esub1) ) + 
            geom_line(mapping=aes(x=year,y=se_f_value,color=country))+ylim(c(0,100))
        if(input$points_Esub1==TRUE){
            graph_se_f =graph_se_f +
                geom_point(data = data_se_f %>% filter(region==input$select_Esub1),aes(x=year,y=se_f_value),color="purple")
        }
        if(input$Esub1legend==FALSE){
            graph_se_f =graph_se_f +
                theme(legend.position = "none")
        }
        ggplotly(graph_se_f)
    ...
    output$plot_se_m = renderPlotly({
        ...
        graph_se_m = ggplot(data = data_se_m %>% filter(data_se_m$region==input$select_Esub2) ) + geom_line(mapping=aes(x=year,y=se_m_value,color=country))
        if(input$points_Esub2==TRUE){
            graph_se_m =graph_se_m +
                geom_point(data = data_se_m %>% filter(region==input$select_Esub2),aes(x=year,y=se_m_value),color="purple")
        }
        if(input$Esub2legend==FALSE){
            graph_se_m =graph_se_m +
                theme(legend.position = "none")
        }
        ggplotly(graph_se_m)
    ...
    output$plot_pr_f = renderPlotly({
        ...
        graph_pr_f = ggplot(data = data_pr_f %>% filter(data_pr_f$region==input$select_Esub3) ) + geom_line(mapping=aes(x=year,y=pr_f_value,color=country))+ylim(c(0,45))
        if(input$points_Esub3==TRUE){
            graph_pr_f =graph_pr_f +
                geom_point(data = data_pr_f %>% filter(region==input$select_Esub3),aes(x=year,y=pr_f_value),color="purple")
        }
        if(input$Esub3legend==FALSE){
            graph_pr_f =graph_pr_f +
                theme(legend.position = "none")
        }
        ggplotly(graph_pr_f)
    ...
    output$plot_pr_m = renderPlotly({
        ...
        graph_pr_m = ggplot(data = data_pr_m %>% filter(data_pr_m$region==input$select_Esub4) ) + geom_line(mapping=aes(x=year,y=pr_m_value,color=country))+ylim(c(40,100))
        if(input$points_Esub4==TRUE){
            graph_pr_m =graph_pr_m +
                geom_point(data = data_pr_m %>% filter(region==input$select_Esub4),aes(x=year,y=pr_m_value),color="purple")
        }
        if(input$Esub4legend==FALSE){
            graph_pr_m =graph_pr_m +
                theme(legend.position = "none")
        }
        ggplotly(graph_pr_m)
